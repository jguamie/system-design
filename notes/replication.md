...
This mode remains available with node failures due to the conditions `W < N` and `R < N`. Reads and writes are always sent to all *N* replicas in parallel. The client waits for *W* or *R* nodes to respond before a write or read is considered successful.
# References
